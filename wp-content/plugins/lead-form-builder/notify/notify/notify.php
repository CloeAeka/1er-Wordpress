 ...
 * @return bool|null
 */
function thunk_companion_admin_notice(){
  global $current_user;
  ...
  if ( !get_user_meta( $user_id, esc_html( $theme_data->get( 'TextDomain' ) ) . '_notice_ignore' ) ) {
    ...
    <div class="thunk-notice">
        ...
        <a href="https://themehunk.com/product/big-store/" target="_blank">
            <img src="<?php echo LFB_PLUGIN_URL.'notify/assets/images/plugin-banner.png';?>" alt="">
          </a>
        </div>
...
add_action( 'admin_notices', 'thunk_companion_admin_notice' );

add_action( 'admin_init', 'thunk_companion_notice_ignore' );
function thunk_companion_notice_ignore(){
  global $current_user;
  $theme_data  = wp_get_theme();
  $user_id   = $current_user->ID;
...
function themehunk_companion_notice_scripts() {  
            wp_enqueue_style('hunk-companion-notices', LFB_PLUGIN_URL.'notify/assets/css/notice.css');
        }
add_action('admin_enqueue_scripts', 'themehunk_companion_notice_scripts');